...
INITSCRIPT_PACKAGES = "${PN}-run-domd ${PN}-run-domu ${PN}-run-domf ${PN}-run-vcpu_pin ${PN}-run-set_root_dev"
...
INITSCRIPT_PARAMS_${PN}-run-domu = "defaults 87"
...
do_install() {
    ...
    install -m 0744 ${WORKDIR}/${DOMU_CONFIG} ${D}${base_prefix}${XT_DIR_ABS_ROOTFS_DOM_CFG}/domu-salvator-x-h3.cfg
